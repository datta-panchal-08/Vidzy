...
const VideoCard = ({ video }) => {
  ...
  return (
    <div className='w-[100%] cursor-pointer hover:bg-pink-50 duration-300 rounded-lg'>
      ...
      <div className="content-and-profile mt-2 w-full flex  gap-3">
        ...
        <div className="channel-data">
          ...
          <h4 className='text-[0.9rem] text-zinc-800 font-[400]'>{snippet?.channelTitle}</h4>
          <div className='flex items-center gap-1'>
            <h4 className='text-[0.8rem] text-zinc-700 font-[400]'>{formatViews(views)} views</h4>
            <div className='w-[0.2rem] h-[0.2rem] bg-zinc-500 rounded-lg'></div>
            <h4 className='text-[0.8rem] text-zinc-700 font-[400]'>{moment(snippet?.publishedAt).fromNow()}</h4>
          </div>
        </div>
      </div>
    </div>
